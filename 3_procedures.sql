...
CREATE OR REPLACE PACKAGE BODY billing_pkg AS
    ...
    AS
        ...
        v_user_id           api_users.user_id%TYPE;
        v_pricing_model_id  subscription.pricing_model_id%TYPE;
        v_discount          subscription.discount%TYPE;
        v_usage_tracking_id usage_tracking.tracking_id%TYPE;
        ...
        -- Custom exceptions
        e_subscription_not_found     EXCEPTION;
        e_pricing_model_not_found    EXCEPTION;
        e_user_not_found             EXCEPTION;
        e_api_not_found              EXCEPTION;
    ...
    BEGIN
        ...
        IF NOT subscription_exists(p_subscription_id) THEN
            RAISE e_subscription_not_found;
        END IF;

        -- Retrieve pricing model ID, tracking id, discount for the subscription
        SELECT pricing_model_id, NVL(discount, 0), tracking_id
        INTO v_pricing_model_id, v_discount, v_usage_tracking_id
        FROM subscription
        WHERE subscription_id = p_subscription_id;

        -- Check if the pricing model exists
        IF NOT pricing_model_exists(v_pricing_model_id, p_api_id) THEN
            RAISE e_pricing_model_not_found;
        END IF;

        -- Retrieve model_type, request_limit, rate from pricing_model
        SELECT model_type, rate, request_limit
        INTO v_model_type, v_rate, v_request_limit
        FROM pricing_model 
        WHERE model_id = v_pricing_model_id;

        -- Retrieve request_count from usage_tracking
        SELECT request_count
        INTO v_request_count
        FROM usage_tracking
        WHERE tracking_id = v_usage_tracking_id;

        -- Check if the request limit matches the request count for "Subscription" model
        IF v_model_type = 'Subscription' AND v_request_limit <> v_request_count THEN
            RAISE e_request_count_mismatch;
        END IF;

        -- Calculate the total amount for Subscription model type
        IF v_model_type = 'Subscription' THEN
        ...
        -- Success message
        p_message := 'Billing record generated successfully with total amount: ' || v_total_amount;
    ...
    EXCEPTION
        ...
        WHEN e_subscription_not_found THEN
            p_message := 'Subscription does not exist.';
            ROLLBACK;

        WHEN e_pricing_model_not_found THEN
            p_message := 'Pricing model does not exist.';
            ROLLBACK;
